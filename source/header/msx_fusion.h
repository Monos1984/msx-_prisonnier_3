...
void 				PrintNumber(unsigned int value);								// Print a number on Screen to Text screen mode
...
char				Getcon(void);													//  get char from input and display, \n goes next line, BS previous 
...
/*	VDP functions	
*/
#define 			IsVsync()	((VDPstatus( 0 ) & 0x80) != 0)
#define 			IsHsync()	((VDPstatus( 1 ) & 0x01) != 0)
...
void 				SetDisplayPage( unsigned char disp_page );					// Set the Display Page
void 				SetActivePage( unsigned char act_page );					// Set the active Page
...
void 				FillVram(int Startaddress, char value, int length);			// Fill the Vram from startadress, with Value, for Length bytes
void 				PutText( int X, int Y,  char *str );						// Print Text string on Graphic screen
...
int	         		WaitForKey( void );											// Wait for key-press, returns key code 
int		        	Inkey( void );												// Checks key-press, returns key code, otherwise 0 
char         		WaitKey(void);                        						// Wait for a key press. Return Ascii Code of key
void         		KillKeyBuffer(void);                   						// Erase key buffer
char          		KeyboardHit (void);         								// checks keypress */
int          		keyboardRead( void );       								// verifies MSX keyboard status by ports 0xAA,0xA9 


/* --------------------------------------------------------- */
/*	String functions										 */
char				StrToLower	( char c );										// convert to lowercase 
char				StrToUpper	( char c );										// convert to uppercase 
...
void 				IntSwap(int *a, int *b);									// Swap two Int Varaibales
...
/* --------------------------------------------------------- */
/* Interupt	functions										 */
#define				EnableInterupt()	{ __asm ei;   __endasm; }				// Enable and Disable Interupt 
#define				DisableInterupt()	{ __asm di;   __endasm; }
...
int					SetDate (DATE *date);  										// set date, returns 0 if valid 
